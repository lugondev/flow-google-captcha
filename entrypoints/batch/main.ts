...
function readSettings(): void {
  settings.mediaType = ($('batch-kind') as HTMLSelectElement).value as Settings['mediaType'];
  settings.model = ($('batch-img-model') as HTMLSelectElement).value;
  settings.videoModel = ($('batch-vid-model') as HTMLSelectElement).value;
  const vlen = ($('batch-vid-len') as HTMLSelectElement).value;
  settings.videoLengthSeconds = vlen ? Number(vlen) : undefined;
  settings.orientation = ($('batch-orient') as HTMLSelectElement).value as Settings['orientation'];
  settings.count = Math.max(1, Number(($('batch-count') as HTMLInputElement).value) || 1);
  settings.maxAttempts = Math.max(1, Number(($('batch-attempts') as HTMLInputElement).value) || 3);
  settings.concurrency = Math.min(5, Math.max(1, Number(($('batch-concurrency') as HTMLInputElement).value) || 3));
  updateSettingsSummary();
  saveBatch();
}
function applySettingsToInputs(): void {
  ($('batch-kind') as HTMLSelectElement).value = settings.mediaType;
  ($('batch-img-model') as HTMLSelectElement).value = settings.model;
  ($('batch-orient') as HTMLSelectElement).value = settings.orientation;
  ($('batch-count') as HTMLInputElement).value = String(settings.count);
  ($('batch-attempts') as HTMLInputElement).value = String(settings.maxAttempts);
  ($('batch-concurrency') as HTMLInputElement).value = String(settings.concurrency);
  toggleModelFields();
}
...
function toggleModelFields(): void {
  ...
  ($('batch-fld-img-model') as HTMLElement).style.display = isVideo ? 'none' : '';
  ($('batch-fld-vid-model') as HTMLElement).style.display = isVideo ? '' : 'none';
  const fam = videoFamilies.find((f) => f.id === settings.videoModel);
  const showLen = isVideo && !!fam && fam.durations.length > 1;
  ($('batch-fld-vid-len') as HTMLElement).style.display = showLen ? '' : 'none';
}

function fillVideoDurations(): void {
  const sel = $('batch-vid-len') as HTMLSelectElement;
  const fam = videoFamilies.find((f) => f.id === settings.videoModel);
...
function populateVideoModels(): void {
  chrome.runtime.sendMessage({ type: 'GET_VIDEO_MODELS', projectId: selProjectId || undefined }, (data) => {
    ...
    const sel = $('batch-vid-model') as HTMLSelectElement;
...
function render(): void {
  ...
  $('batch-count-pill')!.textContent = `${rows.length} dòng`;
}
...
function setRunUI(on: boolean): void {
  ($('batch-run-all') as HTMLButtonElement).disabled = on || !flowReady;
  ($('batch-run-all') as HTMLButtonElement).textContent = on ? '⏳ Đang chạy…' : '▶ Chạy tất cả';
  const stopBtn = $('batch-stop-all') as HTMLButtonElement | null;
...
function clearBatch(): void {
  ...
  render();
  renderGlobalRefs();
  ($('batch-project') as HTMLSelectElement).value = '';
  ($('batch-workflow') as HTMLSelectElement).innerHTML = '<option value="">(mặc định)</option>';
  ($('batch-wf-manual') as HTMLInputElement).value = '';
}
...
function reflectReady(): void {
  if (!running) ($('batch-run-all') as HTMLButtonElement).disabled = !flowReady;
}

function fetchStatus(): void {
  chrome.runtime.sendMessage({ type: 'STATUS' }, (data) => {
    if (chrome.runtime.lastError || !data) return;
    const el = $('batch-token')!;
...
function populateProjects(): void {
  chrome.runtime.sendMessage({ type: 'GET_PROJECTS' }, (data) => {
    if (chrome.runtime.lastError || !data) return;
    const sel = $('batch-project') as HTMLSelectElement;
...
function loadWorkflows(pid: string): void {
  const sel = $('batch-workflow') as HTMLSelectElement;
...
function updateSettingsSummary(): void {
  const modelLabel = ($('batch-img-model') as HTMLSelectElement).selectedOptions[0]?.textContent || settings.model;
  ...
  $('batch-settings-summary')!.innerHTML =
...
function openRefModal(target: RefTarget): void {
  refTarget = target;
  $('batch-ref-modal-title')!.textContent = target === 'global'
    ? 'Ảnh style chung (áp cho mọi dòng)'
    : `Ảnh tham chiếu — dòng ${rows.indexOf(target) + 1}`;
  $('batch-ref-pg')!.innerHTML = '';
  renderModalRefs();
  $('batch-ref-modal')!.classList.add('open');
}
function closeRefModal(): void { $('batch-ref-modal')!.classList.remove('open'); refTarget = null; }

function renderModalRefs(): void {
  const refs = targetRefs();
  if (!refs) return;
  $('batch-ref-modal-hint')!.textContent = `${refs.length}/${MAX_REFS} ảnh.`;
  $('batch-ref-current')!.innerHTML = refs.map((r, i) => {
    const src = r.thumb || r.url || '';
  ...
  document.querySelectorAll<HTMLElement>('#batch-ref-pg .pm').forEach((el) => {
...
function renderGlobalRefs(): void {
  ...
  }).join('') + '<span class="ref-add" id="batch-global-add" title="Thêm ảnh style chung">+</span>';
  $('batch-global-hint')!.textContent = globalRefs.length
    ? `${globalRefs.length} ảnh · tự ghép vào mọi dòng khi chạy`
    : 'Chưa có — bấm + để thêm';
  $('batch-global-add')!.addEventListener('click', () => openRefModal('global'));
...
function toggleProjectRef(m: ProjectMedia): void {
  ...
  else if (refs.length >= MAX_REFS) { $('batch-ref-modal-hint')!.textContent = `Tối đa ${MAX_REFS} ảnh.`; return; }
...
async function addUploadsToActive(files: FileList): Promise<void> {
  ...
  for (const file of Array.from(files)) {
    ...
    if (refs.length >= MAX_REFS) { $('batch-ref-modal-hint')!.textContent = `Tối đa ${MAX_REFS} ảnh.`; break; }
...
function addRowsFromPaste(): void {
  const ta = $('batch-paste') as HTMLTextAreaElement;
...
function deleteSelected(): void {
  const keep: Row[] = [];
  document.querySelectorAll<HTMLTableRowElement>('#batch-rows tr[data-row]').forEach((tr) => {
  ...
  rows = keep;
  ($('batch-chk-all') as HTMLInputElement).checked = false;
  saveBatch(); render();
}

// ─── event wiring ──
function initToolbar(): void {
  $('batch-add-paste')!.addEventListener('click', addRowsFromPaste);
  $('batch-add-row')!.addEventListener('click', addEmptyRow);
  $('batch-del-selected')!.addEventListener('click', deleteSelected);
  $('batch-run-all')!.addEventListener('click', () => {
    if (!flowReady) { fetchStatus(); return; }
    readSettings();
    const targets = rows.filter((r) => r.status !== 'done' && r.prompt.trim());
    void runMany(targets, true);
  });
  $('batch-stop-all')!.addEventListener('click', () => {
    if (!running) return;
    batchCancelled = true;
    rows.forEach((r) => { if (r.status === 'queued') { r.status = 'idle'; updateRow(r); } });
  });
  $('batch-clear')!.addEventListener('click', clearBatch);
  ['batch-img-model', 'batch-orient', 'batch-count', 'batch-attempts', 'batch-concurrency', 'batch-vid-len'].forEach((id) =>
    $(id)!.addEventListener('change', readSettings),
  );
  $('batch-kind')!.addEventListener('change', () => {
    readSettings();
    if (settings.mediaType === 'video' && !videoFamilies.length) populateVideoModels();
    else toggleModelFields();
  });
  $('batch-vid-model')!.addEventListener('change', () => {
    settings.videoModel = ($('batch-vid-model') as HTMLSelectElement).value;
    fillVideoDurations();
    readSettings();
  });
  $('batch-settings-toggle')!.addEventListener('click', () => $('batch-settings-card')!.classList.toggle('open'));

  // Project-bar actions (footer just shows token status + credit now)
  $('batch-open')!.addEventListener('click', () => chrome.runtime.sendMessage({ type: 'OPEN_FLOW_TAB' }));
  $('batch-open-wf')!.addEventListener('click', () => chrome.tabs.create({ url: chrome.runtime.getURL('workflow.html') }));
  $('batch-media')!.addEventListener('click', () => openMediaModal(selProjectId || undefined));
  $('batch-refresh')!.addEventListener('click', () => { populateProjects(); populateVideoModels(); fetchProjectInfo(); fetchStatus(); });
  $('batch-project')!.addEventListener('change', (e) => {
    selProjectId = (e.target as HTMLSelectElement).value;
    selWorkflowId = '';
    ($('batch-wf-manual') as HTMLInputElement).value = '';
    if (selProjectId) loadWorkflows(selProjectId);
    else ($('batch-workflow') as HTMLSelectElement).innerHTML = '<option value="">(mặc định)</option>';
    populateVideoModels();
    saveBatch();
  });
  $('batch-workflow')!.addEventListener('change', (e) => {
    selWorkflowId = (e.target as HTMLSelectElement).value;
    ($('batch-wf-manual') as HTMLInputElement).value = '';
    saveBatch();
  });
  $('batch-wf-manual')!.addEventListener('input', (e) => {
    const v = (e.target as HTMLInputElement).value.trim();
    selWorkflowId = v.match(/\/edit\/([0-9a-f-]{36})/)?.[1] || v;
    if (v) ($('batch-workflow') as HTMLSelectElement).value = '';
    saveBatch();
  });
  $('batch-rename')!.addEventListener('click', () => {
    const pid = selProjectId || tabProjectId;
    if (!pid) { alert('Chọn một project (hoặc mở tab Flow) trước khi đổi tên.'); return; }
    const projSel = $('batch-project') as HTMLSelectElement;
    const cur = selProjectId ? (projSel.selectedOptions[0]?.textContent || '').trim() : '';
    const name = window.prompt('Tên project mới:', cur);
    if (name == null) return;
    const title = name.trim();
    if (!title) return;
    const btn = $('batch-rename') as HTMLButtonElement;
    btn.disabled = true;
  ...
  $('batch-chk-all')!.addEventListener('change', (e) => {
    const on = (e.target as HTMLInputElement).checked;
    document.querySelectorAll<HTMLInputElement>('#batch-rows input[data-act="sel"]').forEach((c) => (c.checked = on));
  });
}
...
function initRefModal(): void {
  $('batch-ref-modal-close')!.addEventListener('click', closeRefModal);
  $('batch-ref-modal')!.addEventListener('click', (e) => { if (e.target === e.currentTarget) closeRefModal(); });
  $('batch-ref-upload')!.addEventListener('click', () => ($('batch-ref-file') as HTMLInputElement).click());
  $('batch-ref-file')!.addEventListener('change', (e) => {
    const files = (e.target as HTMLInputElement).files;
    if (files) void addUploadsToActive(files);
    (e.target as HTMLInputElement).value = '';
  });
  $('batch-ref-load-project')!.addEventListener('click', loadProjectGrid);
  $('batch-ref-current')!.addEventListener('click', (e) => {
